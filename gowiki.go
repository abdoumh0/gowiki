...
var interLink = regexp.MustCompile(`(?im)\[(.*)]`)
...
func (p *Page) Parse() template.HTML {
    return template.HTML(interLink.ReplaceAll(p.Body, []byte(string("<a href='/view/$1'>$1</a>"))))
}
...
func main() {
    ...
    http.Handle("/static/", http.StripPrefix("/static/", http.FileServer(http.Dir("./tmpl/static"))))
